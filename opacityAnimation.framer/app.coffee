...
sendToBottom = (item) ->
		...
		#print(currentImage)
		currentImage.x = (count * (currentImage.width+5)) - 7
...
inputRest = new Layer
	midX:screenTemplate.width/2, y:400, width:156, height:156,backgroundColor: "teal"
	borderRadius: 100
inputRest.shadowX = 5
inputRest.shadowY =5
inputRest.shadowColor = "black"
inputRest.shadowBlur = 10

#inputRest.on "change:x", ->

fadeOutAnimation = new Animation inputRest,
	#opacity: .5
	#delay: .5
	#scale:1
	x:inputRest.x+100
...
# 	delay:.7
fadeInAnimation = new Animation inputRest,
	#opacity: 1
	#scale:1
	x:inputRest.x-100
	curve: Spring(damping: .5)
...
# 	time:2
# moveRight = new Animation inputRest,
# 	x:inputRest.x+100
# 	delay:.7
# moveLeft = new Animation inputRest,
# 	x:inputRest.x-100
# 	time:2
...
#   
#   delta =
#     x: inputRest.midX - event.clientX
#     y: inputRest.midY - event.clientY
#   
#   dist = Math.abs(delta.x) + Math.abs(delta.y)
#   alpha = Utils.modulate dist, [0, 150], [0, .4], true
#   
#   inputRest.shadowX = Utils.modulate delta.x, [0, screenTemplate.width /2], [0, 50]
#   inputRest.shadowY = Utils.modulate delta.y, [0, screenTemplate.height/2], [0, 50]
#   inputRest.shadowBlur = Utils.modulate dist, [0, 100], [5, 10]
#   inputRest.shadowColor = "rgba(0,0,0,#{alpha})"
...
dayPager.snapToPage(sketch3.$3, false)
#minutePager.snapToPage(sketch3.l5,false)
timePager.snapToPage(sketch3.$1pm,false)
